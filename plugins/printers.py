...
class gcustom:
    name = "gcustom"
    description = "Gwen custom output format"
    category = ["default"]
    priority = 99
	...
	print "Total time: %s" % str(summary.totaltime)[:8]

	if summary.backfeed > 0:
	    print "Processed Requests: %s (%d + %d)" % (str(summary.processed)[:8], (summary.processed - summary.backfeed), summary.backfeed)
	else:
	    print "Processed Requests: %s" % (str(summary.processed)[:8])
	print "Filtered Requests: %s" % (str(summary.filtered)[:8])
	print "Requests/sec.: %s\n" % str(summary.processed/summary.totaltime if summary.totaltime > 0 else 0)[:8]
